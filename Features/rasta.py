...
def get_rasta_d_d(audio_path, win_time, hop_time, order):

    audio_signal,sr = librosa.load(audio_path, sr = None)

    label_file = audio_path[:-4] + '.csv'

    if 'Train' in audio_path:
        noise_label_file = audio_path[:-10] + '_Noise_Train.csv'
    elif 'Test' in audio_path:
        noise_label_file = audio_path[:-9] + '_Noise_Test.csv'
    else:
        print('Noise labels not found')
        exit()

    rasta_feat = rastaplp(audio_signal, fs = sr, win_time = win_time, hop_time = hop_time, modelorder = order)

    rasta_delta = librosa.feature.delta(rasta_feat, axis = 1)
    rasta_delta_delta = librosa.feature.delta(rasta_feat, order = 2, axis = 1)

    output_rasta = np.concatenate((rasta_feat, rasta_delta, rasta_delta_delta), axis = 0)

    output_rasta = output_rasta.T

    total_num_frames = output_rasta.shape[0]

    label_list = np.zeros((total_num_frames,), dtype=np.int16)
    noise_label_list = np.zeros((total_num_frames,), dtype=np.int16)

    time_slot = win_time / 2
    time_step = hop_time

    time_stamps = pd.read_csv(label_file)
    noisy_timestamps = pd.read_csv(label_file)

    for t in range(output_rasta.shape[0]):
        label_list[t] = find_label(time_stamps, time_slot)
        noise_label_list[t] = find_label(noisy_timestamps, time_slot)
        time_slot += time_step

    bool_label_list = label_list == 1
    bool_noise_label_list = np.logical_not(noise_label_list == 1)

    clean_speech_labels = bool_label_list & bool_noise_label_list
    clean_speech_labels = clean_speech_labels.astype(np.int16)

    features_file = audio_path[:-4] + '_rastaFeat.npy'
    out_labels_file = audio_path[:-4] + '_rastaLabel.npy'
    clean_labels_file = audio_path[:-4] + '_rastaCleanLabel.npy'

    np.save(features_file, output_rasta)
    np.save(out_labels_file,label_list)
    np.save(clean_labels_file,clean_speech_labels)

    return None

def find_label(timestamps, time_slot):

    start_slots = np.asarray(timestamps['start'])
    end_slots = np.asarray(timestamps['end'])

    total_intervals = len(timestamps)

    n = 0

    while((end_slots[n] < time_slot)):
        n += 1
        if (n == total_intervals):
            return 0

    label = 1 if (start_slots[n] < time_slot) else 0 

    return label

if __name__ == "__main__":
    audio_files = glob.glob(os.path.normpath('./Data_aug/*.wav'))
    
    for file in tqdm(audio_files,total = len(audio_files)):
        get_rasta_d_d(audio_path = file, win_time = 0.032, hop_time = 0.016, order = 18)
...
from Models.WAD import LSTM_WAD
from Models.Data import Whisper_sequence, Bidir_whisper_sequence, early_stopper
...
def main(data_path):
    ...
    data_files = glob.glob(os.path.normpath(os.path.join(data_path,'*Train_rastaFeat.npy')))
    ...
    scaler_path = os.path.normpath('../CWAD/Checkpoints/LSTM_Scaler.sav')
    ...
    #model = LSTM_WAD(input_size = num_features, layers_size = 128, output_size = 1, num_lstm_layers = 2, sequence_length = sequence_length, bidirectional = True, look_ahead = look_ahead)
    model.to(device)
    ...
    #dataset = Bidir_whisper_sequence(data = data_train, labels = labels_train, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = train_segments_len)
    ...
    #dataset_val = Bidir_whisper_sequence(data = data_val, labels = labels_val, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = val_segments_len)
    ...
    model_path = os.path.normpath('../CWAD/Checkpoints/LSTM_128_LongDS_3020_mps.pt')
    ...
    if (load_pretrained and os.path.exists(model_path)):
        ...
        model_path = os.path.normpath('../CWAD/Checkpoints/BiLSTM_32_LongDS.pt')
        torch.save(model.state_dict(), model_path)
    ...
    test_files = glob.glob(os.path.normpath(os.path.join(data_path,'*Test_rastaFeat.npy')))
    ...
    test_dataset = Bidir_whisper_sequence(data = data_test, labels = label_test, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = test_segments_len)
    ...
    dataset_10db = Bidir_whisper_sequence(data = test_10db, labels = test_lab_10db, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = test_segments_10db)
    ...
    dataset_5db = Bidir_whisper_sequence(data = test_5db, labels = test_lab_5db, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = test_segments_5db)
    ...
    dataset_0db = Bidir_whisper_sequence(data = test_0db, labels = test_lab_0db, sequence_length = sequence_length, look_ahead = look_ahead, segment_lengths = test_segments_0db)
...
if __name__ == "__main__":
    ...
    data_path = os.path.normpath('../CWAD/Dataset')
